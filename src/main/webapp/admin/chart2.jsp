...
<!-- https://brunch.co.kr/@chulhochoiucj0/17   RGB 색 참고 사이트 -->
	...
	<div id="table">
		...
		<div>
				...
				<button id="selectTotalInOfYear" type="button">입력</button>
		</div>
		<div>
			<span id="selectTotalInOfYearTableResult"></span>
		</div>
			<!-- chart -->
	 	<div id="newChart">
...
<script>
	<!-- chart -->
$('#selectTotalInOfYear').click(function(){
	$('#Mychart2').remove();<!-- 전의 chhart랑 겹치게 나와 전 차트지움 -->
	//chart.js 동적 크기
	$('#newChart').append('<canvas id="chart2" style="height:40vh; width:70vw"></canvas>'); 
	$.ajax({
		url:'/selectTotalInOfYear/'+$('#year').val(),
		type:'get',
...
$('#selectTotalInOfYear').click(function(){
	$.ajax({
		...
		success:function(data){
			...
			`;
			$('#selectTotalInOfYearTableResult').html(html);
		}
	});
});
</script>
